...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    for(float i = 0.0; i < numRays; i++) {
        ...
        vec3 rayCol = rayChange.x + rayChange.y * cos(rayColor * 10.0 + i * rayChange.z + t);
    ...
    float n = hash12(id);
    if(n > 0.92) {
        float size = sin(t * 3.0 + n * 10.0) * 0.5 + 0.5;
        float star = smoothstep(0.1 * size, 0.0, length(fract(gv) - 0.5));
        col += star * vec3(1.0, 0.9, 0.7) * n;
    }
