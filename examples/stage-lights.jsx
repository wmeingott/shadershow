...
export default function StageLights() {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      const spot = new THREE.SpotLight(color, 50, 25, 0.4, 0.5, 1);
    ...
    for (let i = 0; i < 6; i++) {
      ...
      const spot = new THREE.SpotLight(color, 80, 30, 0.25, 0.3, 1);
    ...
    // WASH LIGHTS - Side trusses
    const washColors = [0xff3366, 0x3366ff, 0xff3366, 0x3366ff];
    const washPositions = [
      [-14, 10, 4], [-14, 10, -4], [14, 10, 4], [14, 10, -4]
    ];
    ...
    function animate() {
      ...
      movingHeads.forEach((mh, i) => {
        const pan = Math.sin(time * 0.5 + mh.phase) * 0.8;
        const tilt = Math.sin(time * 0.7 + mh.phase * 1.5) * 0.3 + 0.5;
        
        // Update fixture head rotation
        ...
        // Update light target
        const targetX = mh.baseX + Math.sin(time * 0.5 + mh.phase) * 8;
        const targetZ = Math.sin(time * 0.7 + mh.phase * 1.5) * 5;
      ...
      barLights.forEach((bl) => {
        bl.bar.lights.forEach((led, i) => {
          const hue = (time * 0.3 + i * 0.05 + bl.phase) % 1;
      ...
      const blinderOn = Math.sin(time * 2) > 0.9;
      ...
      spotLights.forEach((sl, i) => {
        ...
        sl.light.intensity = 30 + pulse * 40;
  ...
  return (
    <div className="w-full h-screen bg-black flex flex-col">
      <div className="absolute top-4 left-4 z-10 text-white bg-black/50 p-3 rounded-lg text-sm">
        <h2 className="font-bold mb-2">Stage Lighting Simulator</h2>
        <ul className="space-y-1 text-xs text-gray-300">
          <li>• <span className="text-yellow-400">Spots:</span> 6x front wash with color</li>
          <li>• <span className="text-cyan-400">Moving Heads:</span> 6x automated pan/tilt</li>
          <li>• <span className="text-green-400">LED Bars:</span> RGB floor uplighters</li>
          <li>• <span className="text-pink-400">Wash:</span> 4x side color wash</li>
          <li>• <span className="text-white">Blinders:</span> 4-cell audience flash</li>
        </ul>
        <p className="mt-2 text-xs text-gray-400">Click to toggle camera rotation</p>
      </div>
