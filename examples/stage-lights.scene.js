...
function animate(time, delta, params, objects, mouse, channels) {
  const { movingHeads, spotLights, barLights, blinder, haze, hazeMat, hazeCount, cameraAngle } = objects;
  ...
  if (params.autoRotate === 1) {
    ...
    // Get camera from scene's parent (renderer's camera)
    // Note: camera reference is available via the renderer
  }
}
