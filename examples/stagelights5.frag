...
// @param haze float 0.05 [0.0,1.0]
// @param environment color [0.0, 0.0, 0.0]
// @param halo vec2 [0.02,0.15] 
// @param position vec2 [0.0,0.45]
// @param space float 0.13 [0.0,1.0]
// @param fog color [0.05, 0.08, 0.2]
// @param base_sum vec2 [0.0,0.0] [-0.5,0.5]
/* @param base vec2[LIGHTS] [ >>
  [-0.5, -0.45],
  [-0.10, -0.48],
  [-0.5, -0.45],
  [-0.5, -0.45],
...
vec3 drawSpotlight(vec2 uv, vec2 origin, vec2 baseTarget, vec3 color, float animOffset) {
    ...
    float flare = haze * 0.005 / (flareDist + 0.002);
    // Add specular starburst effect slightly
    flare += 0.002 / (abs(p.x * p.y) + 0.005 * halo[1]) * smoothstep(halo[0], 0.0, flareDist);
    accum += color * flare;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    for(int i = 0; i < nr; i++){
      col += drawSpotlight(uv, vec2(space * float(nr / 2 - i)  , 0.00) + position, base[i] + base_sum, colors[i], phase * float(nr / 2 - i));
    }
