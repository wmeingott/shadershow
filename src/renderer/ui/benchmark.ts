// Benchmark module — GPU + CPU performance testing
...
const GPU_FRAMES = 120;
const CPU_ITERATIONS = 3;
...
function computeScore(gpuResults: number[][], cpuResults: number[]): number {
  ...
  for (let si = 0; si < SHADERS.length; si++) {
    for (let ri = 0; ri < RESOLUTIONS.length; ri++) {
      ...
      const ref = REFERENCE_FPS[si]! * (1080 / RESOLUTIONS[ri]!.height);
...
async function runTests(onProgress: (info: ProgressInfo) => void): Promise<BenchmarkResults> {
  // Create benchmark canvas — must be in the DOM (offscreen canvases get GPU-optimized away)
  const canvas = document.createElement('canvas');
  canvas.width = 640;
  canvas.height = 360;
  canvas.style.cssText = 'position:fixed;top:0;left:0;width:1px;height:1px;opacity:0.01;pointer-events:none;z-index:-1';
  ...
  const totalSteps = SHADERS.length * (1 + RESOLUTIONS.length);
  let step = 0;
  let cancelled = false;

  const cpuResults: number[] = [];
  const gpuResults: number[][] = [];
  const gl = renderer.gl;

  // Pre-allocate readPixels buffer — reading 1 pixel forces a full GPU sync
  const syncPixel = new Uint8Array(4);
  const gpuSync = (): void => gl.readPixels(0, 0, 1, 1, gl.RGBA, gl.UNSIGNED_BYTE, syncPixel);
  ...
  try {
    ...
    for (let si = 0; si < SHADERS.length; si++) {
      ...
      times.sort((a, b) => a - b);
      cpuResults.push(times[Math.floor(times.length / 2)]!);
      step++;
  ...
  return { cpuResults, gpuResults, score };
}
...
function createDialog(): DialogElements {
...
function showProgressView(body: HTMLDivElement, footer: HTMLDivElement): ProgressUI {
  body.innerHTML = '';
  footer.innerHTML = '';
  ...
  return {
    update({ progress: pct, label: lbl, detail: det }: ProgressInfo): void {
...
function showResultsView(
  ...
  const { cpuResults, gpuResults, score } = results;
  body.innerHTML = '';
  footer.innerHTML = '';
  ...
  const cpuTableBody = document.createElement('tbody');
  for (let si = 0; si < SHADERS.length; si++) {
    const tr = document.createElement('tr');
    const nameTd = document.createElement('td');
    nameTd.textContent = SHADERS[si]!.name;
  ...
  cpuTable.appendChild(cpuTableBody);
...
function formatResultsText(cpuResults: number[], gpuResults: number[][], score: number, rating: Rating): string {
...
export async function runBenchmark(): Promise<void> {
  ...
  try {
    ...
    errDiv.style.cssText = 'color: var(--error-color, #ff5050); padding: 20px;';
    errDiv.textContent = `Benchmark failed: ${err instanceof Error ? err.message : String(err)}`;
